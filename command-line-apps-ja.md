...
* [mac-storage-cleaner](https://github.com/JubaKitiashvili/mac-storage-cleaner) - プレビューファーストのmacOSディスククリーナー：デフォルトでは削除を行わず、審査済みのキャッシュ許可リストのみを自動削除し、リスクの高い項目はゴミ箱へ移動、47件のパステストで検証されたコード内蔵の拒否リストが写真ライブラリ、バックアップ、キーチェーン、SSH鍵をブロックし、すべての操作を監査ログに記録。 [![Open-Source Software][OSS Icon]](https://github.com/JubaKitiashvili/mac-storage-cleaner) ![Freeware][Freeware Icon]
